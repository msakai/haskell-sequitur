...
-- | Since a grammar generated by /SEQUITUR/ has exactly one rule
-- for each non-terminal symbol, a grammar is represented as a
-- a mappping from non-terminal (left-hand side of the rule) to
-- sequnce of symbols (right hand side of the rule).
--
...
-- sequence. Therefore, 'Grammar' type is instances of 'Foldable',
